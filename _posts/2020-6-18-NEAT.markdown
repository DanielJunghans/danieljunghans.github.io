...
<h4>Connections</h4>
The lines connecting the inputs and nodes are called connections. These connections transmit signals in the form of real numbers. Each connection has its own weight or "bias". The purpose of this is to strengthen or weaken a signal. Neural networks learn by evolving their connection weights. As ANNs learn, the signals from important nodes/inputs will become stronger as unimportant signals become weaker. 

<h4>Hidden Layers</h4>
The second type of layer is the hidden layers. The hidden layers come before the output and is where all the computation is done. The nodes that make up the hidden layer



<h4>Output Layer</h4>
finish up these two sections then replace price from the day before with the word "baseline"
